...
mm = 1e-3;
DefineConstant[ cl = {0.5, Name "Input/0Mesh/1ParameterMesh size ( in mm)", Visible 1} ];
cl = cl*mm;
...
Function wire
    ...
    If(defect == 1 && x0 == 0) // changer cette partie là 
        ...
        theta1 = angle - defect_size/(2*t_semi1);
        theta2 = angle + defect_size/(2*t_semi1);
        point1 = newp; Point(point1) = {t_semi1*( Cos(theta1)), (y0 - t_semi1*Sin(theta1)), 0};
        point2 = newp; Point(point2) = {t_semi1*( Cos(theta2)), (y0 - t_semi1*Sin(theta2)), 0};
        
        point3 = newp; Point(point3) = {(t_semi1 + defect_depth)*( Cos(angle)), (y0 - (t_semi1 + defect_depth)*Sin(angle)), 0};
...
Recursive Color Blue {Physical Surface{20, 42, 45};}
If(armour == 1)
    Recursive Color Blue {Physical Surface{43};}
EndIf
Recursive Color Blue {Physical Surface{13, 23, 33};}
Recursive Color Green {Physical Surface{12, 14, 22, 24, 32, 34, 41};}
...
Recursive Color Grey {Physical Surface{50};}
...
If(armour == 0) 
    MeshSize { PointsOf{ Surface{ out(#out()-1)}; } } = R_Tot/4;
    MeshSize { PointsOf{ Surface{out(15+defect),out(16+defect), out(#out()-2)}; } } = cl;
    
EndIf
If(armour == 1) 
    MeshSize { PointsOf{ Surface{ out(#out()-1)}; } } = R_Tot/4;
    MeshSize { PointsOf{ Surface{out(15+defect), out(17+defect), out(18+defect), out(#out()-2) }; } } = cl;
...
If(defect == 0)
    MeshSize { PointsOf{ Surface{out(0), out(1), out(2), out(4), out(5), out(6), out(7), out(9), out(10), out(11), 
                                out(12), out(14)}; } } = cl;
    MeshSize { PointsOf{  Surface{out(4+defect), out(9+defect), out(14+defect), out(3), out(8), out(13)}; } } = cl;
    
EndIf

If(defect == 1)
    MeshSize { PointsOf{ Surface{out(0), out(2), out(4+defect), out(5+defect), out(6+defect), out(7+defect), out(8+defect), 
                                out(9+defect), out(10+defect), out(11+defect), out(12+defect), out(13+defect), out(14+defect)}; } } = cl;

    MeshSize{ PointsOf{Physical Surface{4}; } } = cl;
EndIf
MeshSize { PointsOf{ Physical Line{60};} } = 20*mm; // outer boundary of EMdom
