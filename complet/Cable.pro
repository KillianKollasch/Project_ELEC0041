...
Function {
  ...
  sigma[Ground] = 28; 
  sigma[Region[{Insolating_PET, Insolating_PP, Insolating_PVC, Insolating_XLPE}]] = 10^exponent; // 1e-4 semble bien ou 1e-5 ? 
  
  If(Flag_AnalysisType == 3)
    sigma[Copper] = 5.99e7/(1 + 0.00386*($1 - 20));
    sigma[SteelPipe] = 4.7e6/(1 + 0.006*($1 - 20));
    sigma[DomainS_Mag] = 5.99e7/(1 + 0.00386*($1 - 20));
  Else
    sigma[Copper] = 5.99e7;//1e-6;
    sigma[SteelPipe] = 4.7e6;
    sigma[DomainS_Mag] = 5.99e7;
  EndIf

  epsilon[Region[{Ground, SteelPipe, Ind_1, Ind_2, Ind_3, Copper, Insolating_PET, Insolating_PVC, Insolating_XLPE}]] = eps0;
  epsilon[Semiconductor] = eps0*2.25;
  epsilon[Insolating_PP] = eps0*2;
  ...
  EndIf
    ...
    Freq = {50, Min 1, Max 10000, Step 1,
      Name "Input/2Parameters/2frequency [Hz]"}];
  ...
  Vrms = 45e3; // RMS value in the line voltage [V]
...
If (Flag_AnalysisType == 0)
  ...
  { Name Post_Ele; NameOfPostProcessing EleDyn_v;
    Operation {
      Print[ v,  OnElementsOf Domain_Ele, File "res_electro/v.pos" ];
      Print[ norm_e, OnElementsOf Domain_Ele, Name "|E| [V/m]",  File "res_electro/em.pos" ]; 
      Print[ norm_d, OnElementsOf Domain_Ele, Name "|D| [A/mÂ²]", File "res_electro/dm.pos" ];
      Print[ e,  OnElementsOf Domain_Ele, Name "E [V/m]",  File "res_electro/e.pos" ];
      Print[ d,  OnElementsOf Domain_Ele, Name "D [A/mÂ²]",  File "res_electro/d.pos" ];
      Print[ j,  OnElementsOf Domain_Ele, Name "J [A/m²]",  File "res_electro/j.pos" ];
      //Print[ j,  OnElementsOf Insolating_PP, Name "J2 [A/m²]",  File "res/j2.pos" ];
      Print[ j_dis,  OnElementsOf Domain_Ele, Name "J Displacement [A/m²]",  File "res_electro/j_dis.pos" ];
      Print[ j_all,  OnElementsOf Domain_Ele, Name "J All (cond + displa) [A/m²]",  File "res_electro/j_all.pos" ];

      Print[ ElectricEnergy[Domain_Ele], OnGlobal, Format Table, StoreInVariable $We,
        SendToServer "{01Global ELE results/0Electric energy", File  "res_electro/energy.dat" ];
      Print[ V0, OnRegion Ind_1, Format Table, StoreInVariable $voltage,
        SendToServer "{01Global ELE results/0Voltage", Units "V", File "res_electro/U.dat" ];
      Print[ C_from_Energy, OnRegion DomainDummy, Format Table, StoreInVariable $C1,
        SendToServer "{01Global ELE results/1Capacitance", Units "F/m", File  "res_electro/C.dat" ];
      
    }
  }
  { Name Post_Ele_energy; NameOfPostProcessing EleDyn_v;
    Operation {
      ...
      { Name Post_Mag; NameOfPostProcessing MQS_a_2D;
        Operation {
          ...
          Print[ az, OnElementsOf Domain_Mag,
            Name "flux lines: Az [T m]", File "res_mag/az.pos" ];
          Print[ b, OnElementsOf Domain_Mag,
            Name "B [T]", File "res_mag/b.pos" ];
          Print[ norm_b , OnElementsOf Domain_Mag,
            Name "|B| [T]", File "res_mag/bm.pos" ];
          Print[ jz , OnElementsOf Region[{DomainC_Mag}],
            Name "jz [A/m^2]", File "res_mag/jz_inds.pos" ];
          Print[ norm_j , OnElementsOf DomainC_Mag,
            Name "|j| [A/m^2]", File "res_mag/jm.pos" ];

          // global results
          Print[ global_losses[DomainC_Mag], OnGlobal, Format Table,
            SendToServer "{01Global MAG results/0Losses conducting domain",
            Units "W/m", File > "res_mag/losses_total.dat" ];

          Print[ global_losses[DomainS_Mag], OnGlobal, Format Table,
            SendToServer "{01Global MAG results/0Losses source",
            Units "W/m", File > "res_mag/losses_inds.dat" ];
          Print[ R, OnRegion Ind_1, Format Table,
            SendToServer "{01Global MAG results/1Resistance", Units "Î©/m", File  "res_mag/Rinds.dat" ];
          Print[ L, OnRegion Ind_1, Format Table,
            SendToServer "{01Global MAG results/2Inductance", Units "H/m", File  "res_mag/Linds.dat" ];

          Print[ MagneticEnergy[Domain_Mag], OnGlobal, Format Table, StoreInVariable $Wm,
            SendToServer "{01Global MAG results/3Magnetic energy", File  "res_mag/MagEnergy.dat" ];
          Print[ I0, OnRegion Ind_1, Format Table, StoreInVariable $current,
            SendToServer "{01Global MAG results/4Current", Units "I", File  "res_mag/I.dat" ];
          Print[ L_from_Energy, OnRegion DomainDummy, Format Table, StoreInVariable $L1,
            SendToServer "{01Global MAG results/5Inductance from energy", Units "H/m", File  "res_mag/L.dat" ];
        }
      }
      { Name Post_Mag_losses; NameOfPostProcessing MQS_a_2D;
        Operation {
          // local results
          Print[ jz , OnElementsOf Region[{DomainC_Mag}],
            Name "jz [A/m^2]", File "res_mag/jz_inds.pos" ];
...
If (Flag_AnalysisType == 2)
  ...
  Function {
    ...
    k[Semiconductor] = 10; 
    k[Ground] = 0.4; 
    k[Region[{Insolating_PET, Insolating_PP, Insolating_PVC, Insolating_XLPE}]] = 0.46; 
    ...
    { Name The; NameOfFormulation The_T;
      Quantity {
        ...
        { Name mean_T; Value {
              Integral {
                [ {T}/(Pi*dinf*dinf)];
                In Domain_tot; Jacobian Vol; Integration I1;
              }
            }
        }
      }
  ...
  PostOperation {
    { Name Post_MagTher ; NameOfPostProcessing The ;
      Operation {
        Print[ T, OnElementsOf Domain_Mag , File "res_thermal/map_T.pos"];
        Print[ q, OnElementsOf Domain_Mag , File "res_thermal/map_Q.pos"];
        Print[ sigma, OnElementsOf Conductor , File "res_thermal/map_Sigma.pos"];
        Print[ global_losses[DomainC_Mag], OnGlobal, Format Table,
              SendToServer "{01Global MAG results/0Losses conducting domain  lin ",
              Units "W/m", File > "res_thermal/losses_total_lin.dat" ];
        Print[ global_losses[DomainS_Mag], OnGlobal, Format Table,
              SendToServer "{01Global MAG results/0Losses source  lin ",
              Units "W/m", File > "res_thermal/losses_inds_lin.dat" ];

            Print[ mean_T[Domain_tot], OnGlobal, Format Table,
              SendToServer "{01Global MAG results/1mean temperature  lin ",
               Units "K", File > "res_thermal/mean_T_lin.dat" ];
...
If(Flag_AnalysisType == 3 )
  ...
  PostOperation {
      { Name Post_MagTher_non_lin ; NameOfPostProcessing The_non_lin ;
        Operation {
          Print[ T, OnElementsOf Domain_Mag , File "res_thermal_non_lin/map_T.pos"];
          Print[ q, OnElementsOf Domain_Mag , File "res_thermal_non_lin/map_Q.pos"];
          //Print[ qVol, OnElementsOf Domain_tot , File "map_QVol.pos"];
          Print[ jz , OnElementsOf Region[{DomainC_Mag}],
              Name "jz [A/m^2]", File "res_thermal_non_lin/jz_inds.pos" ];
            Print[ norm_j , OnElementsOf DomainC_Mag,
              Name "|j| [A/m^2]", File "res_thermal_non_lin/jm.pos" ];

            // global results
            Print[ global_losses[DomainC_Mag], OnGlobal, Format Table,
              SendToServer "{01Global MAG results/0Losses conducting domain non lin ",
              Units "W/m", File > "res_thermal_non_lin/losses_total_non_lin.dat" ];

            Print[ global_losses[DomainS_Mag], OnGlobal, Format Table,
              SendToServer "{01Global MAG results/0Losses source non lin ",
              Units "W/m", File > "res_thermal_non_lin/losses_inds_non_lin.dat" ];

              Print[ mean_T[Domain_tot], OnGlobal, Format Table,
              SendToServer "{01Global MAG results/1mean temperature non lin ",
               Units "K", File > "res_thermal_non_lin/mean_T_non_lin.dat" ];

              Print[ sigma, OnElementsOf Conductor , File "res_thermal_non_lin/map_Sigma.pos" ];
        }
      }
    }
