...
Function {
  ...
  I_0[] = ($Time < 1)?( ($Time < 1-tr)? (($Time < tr)? $Time/tr : 1): (1-$Time)/tr ): 0;
}
...
If (Flag_AnalysisType > 0)
  ...
  //------------------------------------------------------------------------
  Function{
          tmax = DefineNumber[2, Min 1, Max 3600, Step 1,
        Name "Parameters/Simulation time [s]" ];
      dt = DefineNumber[0.01, Min 0.01, Max 100, Step 0.01,
...
If (Flag_AnalysisType == 2)
    ...
    { Name The_T ; Type FemEquation;
      ...
      Integral { [ -h[] * T0[] , {T} ];
        ...
        Galerkin { [ -0.5*sigma[]* <a>[SquNorm[Dt[{a}]]] - 0.5*<ir>[SquNorm[{ir}*Ns[]/Sc[]]]/sigma[], {T} ];
                  In Domain_tot_temp; Integration I1; Jacobian Vol;  }
  ...
  PostOperation {
    { Name Post_MagTher ; NameOfPostProcessing The ;
      Operation {
        Print[ T, OnElementsOf Domain_tot_temp , File Sprintf["res/map_T_%g.pos", cl/mm], Name Sprintf["map_T_%g_[C]", cl/mm]];
