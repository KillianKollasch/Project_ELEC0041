...
// If you have surfaces that have common lines and therefore points, then the last instruction prevails
    MeshSize { PointsOf{ Surface{ out(#out()-1)};} } = R_Tot/20;
If(armour == 0) 
...
If(armour == 1) 
    ...
    MeshSize { PointsOf{ Surface{out(15+defect),out(16+defect), out(17+defect), out(18+defect), out(#out()-2), out(#out()-1) }; } } = cl;
...
If(defect == 0)
    MeshSize { PointsOf{ Surface{out(0), out(1), out(2), out(4), out(5), out(6), out(7), out(9), out(10), out(11), 
                                out(12), out(14)}; } } = cl;
    MeshSize { PointsOf{ Surface{out(0), out(1), out(2), out(5), out(6), out(7), out(9), out(11), 
                                out(12), out(10)}; } } = cl/4;
